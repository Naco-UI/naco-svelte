...
<Story of={Stories.Balance} />

## Interactive

Form row can be interactive and can be used as a button. To do that, use `interactive` prop and set `on:click` handler. In this mode, the transferred content is not output. The chevron is always displayed instead.

<Story of={Stories.Interactive} />

## Large content

A group can have an inverted balance when the right side is larger than the left side. To do this, it is necessary to set the `largeContent` property to the group.

<Story of={Stories.LargeContent} />
